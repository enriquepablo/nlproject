...
class KB(object):
    ...
    def get_instancesn(self, *sentences):
        templs = []
        queries = []
        vrs = {}
    ...
    def get_instances(self, *sentences):
        q, templs = self.get_instancesn(*sentences)
        logger.info(q)
        return clips.Eval(q), templs

    def retract(self, sentence):
        for ins in self.get_instances(sentence):
    ...
    def ask(self, *sentences):
        ...
        '''
        clps, templs = self.get_instances(*sentences)
        resp = []
        if clps:
    ...
    def ask_obj(self, sentence):
        '''
        retrieve sentences in clips
        matching the given sentence.
        Can use variables.
        '''
        clps, templs = self.get_instances(sentence)
        sens = []
        if clps:
            for ins in clps:
